  ...
  const renderWidgetContent = useCallback(
    (
      ...
      if (widget.type === "notification") {
        return <NotificationWidget config={widget.config as NotificationConfig} />;
      }
      if (widget.type === "datasetView") {
        ...
        return (
          <div className="w-full h-full overflow-auto p-2 text-xs">
            {config.title && (
              <div className="font-semibold mb-2" style={{ color: config.rowTextColor }}>
                {config.title}
              </div>
            )}
            <table className="w-full border-collapse">
              <thead style={{ backgroundColor: config.headerBackground }}>
                <tr>
                  {config.columns.map((column) => (
                    <th
                      key={column}
                      className="text-left px-2 py-1 border border-white/20"
                      style={{ color: config.rowTextColor }}
                    >
                      {column}
                    </th>
                  ))}
              ...
              <tbody style={{ color: config.rowTextColor }}>
                {config.rows.map((row, i) => (
                  <tr key={`row-${i}`} style={{ backgroundColor: config.rowBackground }}>
                    {row.map((cell, idx) => (
      ...
      if (widget.type === "shellCommand") {
        ...
        return (
          <div
            className="w-full h-full text-green-400 font-mono text-xs p-2 overflow-auto"
            style={{ backgroundColor: config.backgroundColor }}
          >
